...
[ttrpg-codex](https://github.com/pjunak/ttrpg-codex) â€” 313 creature stat blocks
...
(where these monsters originally lived) â€” the DM chooses per campaign which
...
- **`/bestiary` pages** â€” an index sorted by Challenge Rating + a stat-block
  detail page per creature (type, AC, HP, speed, CR, abilities, traits, prose).
- **`[[Name|monster]]` wiki-links** resolve into the detail pages.
- **Data api** â€” `provide()`s `{ apiVersion: 1, listMonsters(q?), getItem,
...
> **Data provenance:** the records were bulk-scraped with a chain-shift
> corruption (wrong monster's stat block under a name) that has since been
> **repaired against the SRD 5.2** â€” including recovering `werewolf`, `wolf`,
> `commoner`, and `bone_devil`. See
> [`data/KNOWN_ISSUES.md`](data/KNOWN_ISSUES.md) for the repair history and
> remaining limitations (some records carry rules content without book
> flavor prose).
...
`/item/:kind/:id`, `/kinds`) â€” **no server code, no `server:code` grant, no
...
```sh
# from the ttrpg-codex repo â€” installs this addon locally (bypasses GitHub);
...
# tests (assume ttrpg-codex is a sibling checkout). âš  Windows: use exactly
# this RELATIVE form â€” `node --test tests/` (directory) and absolute paths
# false-fail:
...
Bump `version` in `addon.json` â†’ commit â†’ push. The DM installs/updates from
the GitHub URL via the host's addon wizard (Settings â†’ DoplÅˆky). Because the
manifest declares `contentDir`, content updates are **hot** â€” no server
restart. One rule: "â¬† Aktualizovat vÅ¡e" (update-all) keeps the existing
